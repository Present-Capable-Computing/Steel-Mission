...
def test_application_audit_panel_uses_existing_mutation_ledger_contract():
    chat = runpy.run_path(str(WORKER_DIR / "steel-mission-chat" / "server.py"))
    html = chat["application_chat_index"]()

    assert "auditPanel" in html
    assert "/mutations" in html
    for field in ("Mutation Ledger", "Configuration changes", "Before Hash", "After Hash", "Details"):
        assert field in html
